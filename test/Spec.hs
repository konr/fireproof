...
samplePortfolio = Model.Portfolio 1000.0 60.0
...
logicTests = hspec $ do
  describe "updateAmount" $ do
    it "should update portfolio amount, given interest and inflation" $ do
      (Logic.updateAmount sampleYear samplePortfolio) `shouldBe` (Model.Portfolio 987.00006 60.0)
  ...
  describe "updateFireParams" $ do
    it "should increment year and portfolio amount" $ do
      (Logic.updateFireParams sampleParams) `shouldBe`
        (Model.FireParams (Model.Year 2019 0.03 0.08) (Model.Portfolio 987.00006 60.0))
